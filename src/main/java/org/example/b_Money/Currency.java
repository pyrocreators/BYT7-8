...
public class Currency {
    ...
    public Integer valueInThisCurrency(Integer amount, Currency othercurrency) {
        double amountInOtherCurrency = (double) amount / othercurrency.getRate();

        if (othercurrency.getRate() == 0) {
            throw new IllegalArgumentException("Division by zero: Invalid rate");
        }

        return (int) Math.round(amountInOtherCurrency * rate);
    }
}
